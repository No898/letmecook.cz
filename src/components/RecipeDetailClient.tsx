    ...
    return (
        ...
        >
            <motion.div
                ...
                whileInView={{ scale: 1 }}
                viewport={{ once: true, amount: 0.5 }}
                transition={{ type: "spring", stiffness: 300, damping: 15, delay: index * 0.08 + 0.1 }}
                whileTap={{ scale: 0.9 }}
            >
                {isChecked ? (
                    <motion.svg
...
export default function RecipeDetailClient({ recipe }: RecipeDetailClientProps) {
    ...
    const [procedureView, setProcedureView] = useState<Record<number, 'detailed' | 'brief'>>({});
    ...
    const handleStepCheck = (type: 'detailed' | 'brief', sectionIndex: number, stepIndex: number) => {
    ...
    return (
        ...
        >
            ...
            <div className="text-center mb-12">
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold font-serif mb-4">{recipe.title}</h1>
                {recipe.vietnameseTitle && (
                    <h2 className="text-2xl sm:text-3xl text-gray-500 dark:text-gray-400 font-serif mb-8">{recipe.vietnameseTitle}</h2>
                )}
            </div>

            <div ref={targetRef} className="mb-12 lg:mb-16 relative aspect-video rounded-lg overflow-hidden shadow-xl">
                {recipe.imageUrl && (
                    <motion.div
                        style={{ y }}
            ...
            <p className="text-lg md:text-xl text-gray-700 dark:text-gray-300 mb-12 leading-relaxed max-w-3xl mx-auto text-center">{recipe.description}</p>

            {/* Ikonky pro sdílení a kopírování */}
            <div className="flex justify-center items-center space-x-4 mb-12">
                <span className="text-sm text-gray-500 dark:text-gray-400">Sdílet:</span>
                {/* Facebook */}
                <a
                ...
                {/* WhatsApp */}
                <a
                    href={`https://api.whatsapp.com/send?text=${encodeURIComponent(recipe.title + ' ' + currentUrl)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                {/* Email */}
                <a
                    href={`mailto:?subject=${encodeURIComponent(recipe.title)}&body=${encodeURIComponent('Podívej se na tento recept: ' + currentUrl)}`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-12">
                <div className="lg:col-span-1 space-y-8">
                    {recipe.ingredients.map((section, sectionIndex) => (
                        ...
                        >
                            <h3 className="text-2xl font-semibold font-serif mb-4 border-b-2 border-accent pb-2">{section.sectionTitle}</h3>
                            {section.servings && <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 italic">{section.servings}</p>}
                                ...
                                {section.items.map((item, itemIndex) => {
                                    ...
                                    return (
                                        <ListItem key={key} delay={itemIndex * 0.05}>
                                            <motion.div
                                                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center cursor-pointer transition-colors flex-shrink-0 mt-0.5 ${isChecked ? 'bg-accent border-accent' : 'border-gray-300 dark:border-gray-600 hover:border-accent'}`}
                ...
                <div className="lg:col-span-2 space-y-12">
                    {recipe.procedure.map((section, sectionIndex) => {
                        ...
                        const hasBriefSteps = section.briefSteps && section.briefSteps.length > 0;
                        ...
                        return (
                            ...
                            >
                                <div className="flex justify-between items-center mb-6">
                                    <h3 className="text-3xl font-semibold font-serif">{section.sectionTitle}</h3>
                                    {hasBriefSteps && (
                                        <div className="flex space-x-1 border border-gray-300 dark:border-gray-600 rounded-full p-0.5 text-sm">
                                            <button
                                                ...
                                                className={`px-3 py-1 rounded-full transition-colors ${currentView === 'detailed' ? 'bg-accent text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                            >
                                                Detailní
                                            </button>
                                            <button
                                                onClick={() => toggleProcedureView(sectionIndex)}
                                                className={`px-3 py-1 rounded-full transition-colors ${currentView === 'brief' ? 'bg-accent text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                            >
                                                Stručné
                                            </button>
                                        </div>
                                                ...
                                                {section.steps.map((step, stepIndex) => {
                                                    ...
                                                    return (
                                                        <StepItem
                                                            ...
                                                            onCheck={() => handleStepCheck('detailed', sectionIndex, stepIndex)}
                                                        >
                                                ...
                                                {section.briefSteps.map((step, stepIndex) => {
                                                    ...
                                                    return (
                                                        <StepItem
                                                            ...
                                                            index={stepIndex}
                                                            isChecked={isStepChecked}
                                                            onCheck={() => handleStepCheck('brief', sectionIndex, stepIndex)}
                                                        >
                                                            <p className="text-gray-700 dark:text-gray-300 leading-relaxed">{step}</p>
                                                        </StepItem>
                                                    );
                                ...
                                {section.videoUrl && (
                                    <motion.div
                                        ...
                                        transition={{ duration: 0.5 }}
                                    >
                                        <video controls src={section.videoUrl} className="w-full h-full absolute top-0 left-0 object-cover">
                    ...
                    {recipe.tips && (
                        <motion.section
                            ...
                            transition={{ duration: 0.5 }}
                        >
                            <h3 className="text-3xl font-semibold font-serif mb-6">{recipe.tips.sectionTitle}</h3>
                            <div className="space-y-5">
                                {recipe.tips.items.map((tip, index) => (
                                    <motion.div
                                        ...
                                        <h4 className="font-semibold text-lg mb-2 text-gray-800 dark:text-gray-100">{tip.title}</h4>
                    ...
                    {recipe.serving && (
                        <motion.section
                            ...
                            transition={{ duration: 0.5 }}
                        >
                            <h3 className="text-3xl font-semibold font-serif mb-6">{recipe.serving.sectionTitle}</h3>
