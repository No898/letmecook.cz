...
export default function RecipeCarousel({ recipes, locale }: RecipeCarouselProps) {
    if (!recipes || recipes.length === 0) {
        return null; // Nezobrazovat nic, pokud nejsou recepty
    }

    return (
        <div className="w-full max-w-7xl px-4"> {/* Obal pro šířku a padding */}
            <Swiper
                ...
                {recipes.map((recipe) => (
                    <SwiperSlide key={recipe.id}>
                        ...
                        <Link
                            ...
                            className="block bg-white dark:bg-gray-800 rounded-xl shadow-md dark:shadow-lg overflow-hidden group h-full flex flex-col"
                        >
                            {recipe.imageUrl && (
